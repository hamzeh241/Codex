...
void UpdateRangeBlocksByClose(const double closePrice)
{
   ...
   double buyLimitClose = g_referenceHigh + (2.0 * range);
   double sellLimitClose = g_referenceLow - (2.0 * range);

   if(closePrice > buyLimitClose)
   {
      g_buyBlockedByRangeToday = true;
      CancelPendingByDirection(true);
      g_lastActionInfo = "buy blocked: close > green + 2x range";
   }
   ...
   if(closePrice < sellLimitClose)
   {
      ...
      g_lastActionInfo = "sell blocked: close < red - 2x range";
   }
}
...
void TryPlaceBreakoutPending(const bool isUpBreakout)
{
   ...
   double entryPrice = isUpBreakout ? g_referenceHigh : g_referenceLow;
   double slPrice = isUpBreakout ? g_referenceLow : g_referenceHigh;
   double tpPrice = isUpBreakout ? (entryPrice + (2.0 * range)) : (entryPrice - (2.0 * range));
